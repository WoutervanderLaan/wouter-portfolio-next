  ...
  const addMessage = (message: Omit<MessageBase, "id">) => {
    ...
    setMessages((prev) => {
      const newMessage: StoredMessage = {
        ...
        id: String(crypto.randomUUID()), // local placeholder id
        user_id: String(crypto.randomUUID()), // local placeholder id
      };
        ...
        {(logout) => (
          <div className="flex h-full flex-col gap-4">
            <div className="flex w-full flex-row items-center gap-2">
              ...
              >
                <div
                  className={clsx("h-2 w-2 rounded-full bg-gray-400", {
              ...
              {error && (
                <Text.Small className="text-red-500">{`[${error}]`}</Text.Small>
              )}

              <Button variant="secondary" className="ml-auto" onPress={logout}>
            ...
            >
              ...
              {Object.entries(messages).map(([session, msgArray]) => (
                <Fragment key={session}>
                  <div className="flex flex-col gap-2 p-4">
                    {msgArray.map((message) => (
                      <MessageBalloon.CHAT key={message.id} {...message} />
                    ))}
                  </div>
                  {session !== sessionId && (
              ...
              {!isConnected && <MessageBalloon.WARN text="Disconnected" />}
            </div>
